         ...
         (:run
          (let ((args (cdr code)))
            (cond
              ...
              ((eq (first args) :mount)
               (let ((mount-spec (second args)) (cmd (emit-df (caddr args))))
                 (if (listp mount-spec)
                     (format nil "RUN ~{--mount=~a~^ ~} ~a"
                             (mapcar #'emit-val mount-spec) cmd)
                     (format nil "RUN --mount=~a ~a" (emit-val mount-spec)
                             cmd))))
                ...
                (multiple-value-bind (paths options) (parse-copy-add-args args
                  ...
                  (let ((dest (car (last paths))) (srcs (butlast paths))
                    ...
                    (format nil
                            ...
                            (and chown (emit-val chown)) link
                            (and chmod (emit-val chmod)) parents
                            (and exclude (emit-val exclude))
                            (mapcar #'emit-val srcs) (emit-val dest)))))))
         (:add
          (let ((args (cdr code)))
            (multiple-value-bind (paths options) (parse-copy-add-args args
                                                  '(:chown :link :chmod
              ...
              (let ((dest (car (last paths))) (srcs (butlast paths))
                ...
                (format nil
                        "ADD~@[ --chown=~a~]~:[~; --link~]~@[ --chmod=~a~]~@[ --checksum=~a~] ~{~a~^ ~} ~a"
                        (and chown (emit-val chown)) link
