...
(defun kiro-wrapper-script (real-binary)
  (format nil "#!/usr/bin/env bash~%set -euo pipefail~%~%# Kiro CLI tool trust is controlled via --trust-all-tools / --trust-tools~%# (see 'kiro-cli chat --help'), not via a permissions.yaml file. Inject~%# --trust-all-tools automatically for chat sessions so agents never block~%# on tool-approval prompts, unless the caller already requested specific~%# trust flags. 'chat' is the default subcommand: a bare query (first arg~%# is not one of kiro-cli's known subcommands) also starts a chat session,~%# so treat that case the same way. Known non-chat subcommands are listed~%# below and left untouched to avoid passing an unsupported flag to them.~%known_subcommands=\"debug settings setup update diagnostic init theme issue login logout whoami profile user doctor launch quit restart integrations translate dashboard chat mcp inline agent acp help\"~%args=()~%if [[ \" $* \" != *\" --v3 \"* && \" $* \" != *\" --classic \"* ]]; then~%  args+=(\"--v3\")~%fi~%if [[ ${1:-} == init ]]; then~%  for arg in \"$@\"; do~%    if [[ $arg == --force ]]; then~%      exec ~a \"${args[@]}\" \"$@\"~%    fi~%  done~%  exec ~a \"${args[@]}\" init --force \"${@:2}\"~%fi~%first_arg=${1:-}~%is_known_subcommand=0~%for sub in $known_subcommands; do~%  if [[ $first_arg == \"$sub\" ]]; then~%    is_known_subcommand=1~%    break~%  fi~%done~%if [[ -z $first_arg || $is_known_subcommand -eq 0 || $first_arg == chat ]]; then~%  case \" $* \" in~%    *\" --trust-all-tools \"*|*\" -a \"*|*\" --trust-tools\"*) ;;~%    *) args+=(\"--trust-all-tools\") ;;~%  esac~%fi~%exec ~a \"${args[@]}\" \"$@\"~%"
          real-binary
          real-binary
          real-binary))
