(load (merge-pathnames "dock.lisp" *load-pathname*))
...
(assert-df "RUN cat a.txt | grep -i hello"
           (run (pipe |CAT A.TXT| |GREP -I HELLO|)))

;; Test 4: Raw string reader macro (#r)
(assert-df "RUN echo \"hello world\""
           (run #r(echo "hello world")))
(assert-df "RUN echo \"nested (parentheses)\""
           (run #r[echo "nested (parentheses)"]))
(assert-df "RUN echo \"braces {and} brackets [nested]\""
           (run #r(echo "braces {and} brackets [nested]")))
(assert-df "RUN echo \"custom delimiter\""
           (run #r#echo "custom delimiter"#))
...
(assert-df (format nil "LABEL maintainer=\"me\" \\~%      version=\"1.0\"") (label maintainer "me" version "1.0"))
(assert-df "ONBUILD RUN echo \"onbuild trigger\"" (onbuild (run #r(echo "onbuild trigger"))))
