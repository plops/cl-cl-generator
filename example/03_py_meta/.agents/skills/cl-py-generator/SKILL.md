...
- When adding or modifying test cases in [transpiler-tests.lisp](file:///home/kiel/stage/cl-cl-generator/example/03_py_meta/transpiler-tests.lisp).
- When debugging generated Python files or updating the core emitter in [py.lisp](file:///home/kiel/stage/cl-cl-generator/example/03_py_meta/py.lisp).
...
This example features a self-contained system `:cl-py-generator-example` defined in [cl-py-generator-example.asd](file:///home/kiel/stage/cl-cl-generator/example/03_py_meta/cl-py-generator-example.asd).
...
- **Yield / Yield-From Statements**:
  - Generator yield (empty): `(yield)` or `yield` &rarr; `yield`
  - Generator yield value: `(yield x)` &rarr; `yield x`
  - Generator yield-from value: `(yield-from x)` &rarr; `yield from x`
- **Assert Statements**:
  - Assert without message: `(assert (> x 0))` &rarr; `assert x > 0`
  - Assert with message: `(assert (> x 0) (string "msg"))` &rarr; `assert x > 0, "msg"`
- **Try/Except/Finally**:
  ```lisp
  (try (setf a 1)
       (Exception (setf a 2))
...
- **Decorators & Decorated Definitions**:
  - Explicit Decorator without arguments: `(decorator my_decorator)` &rarr; `@my_decorator`
  - Explicit Decorator with arguments: `(decorator (my_decorator 1 (string "a")))` &rarr; `@my_decorator(1, "a")`
  - Decorated Function:
    ```lisp
    (decorated (my_decorator (my_decorator_with_args 1 2))
               (def foo (x) (return x)))
    ```
    Emits:
    ```python
...
- **Strings**:
  ...
  - F-string: `(fstring "The result is: " (+ x 1) " and status: " (dot status name))` &rarr; `f"The result is: {x + 1} and status: {status.name}"`
  - F-string triple-quoted: `(fstring3 "text: " expr)` &rarr; `f"""text: {expr}"""`
- **Comments**:
...
1. **Running Tests**:
   - Run the script [run-tests.sh](file:///home/kiel/stage/cl-cl-generator/example/03_py_meta/run-tests.sh) to load [transpiler-tests.lisp](file:///home/kiel/stage/cl-cl-generator/example/03_py_meta/transpiler-tests.lisp) and execute `run-transpiler-tests`.
   - The test runner automatically formats code using `ruff format` and checks execution correctness using `python3`.

2. **Adding Test Cases**:
   - Add new cases to `*test-cases*` in [transpiler-tests.lisp](file:///home/kiel/stage/cl-cl-generator/example/03_py_meta/transpiler-tests.lisp) with `:name`, `:description`, `:lisp`, `:python`, and optionally `:exec-test t` and `:expected-output`.
