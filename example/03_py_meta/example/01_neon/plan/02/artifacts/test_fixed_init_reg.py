...
def nominal_frequency_wrapper_fixed_init_reg(nuclear_mass):
    ...
    solver_initial = jaxopt.LBFGS(fun=initial_state_energy_reg, maxiter=500, tol=1e-10, implicit_diff=True)
    ...
    solver_final = jaxopt.LBFGS(fun=final_state_energy_reg, maxiter=500, tol=1e-10, implicit_diff=True)
...
m = 21.0
grad_fn = jax.grad(nominal_frequency_wrapper_fixed_init_reg)
print("Computing jax.grad with fixed init mass and regularized energy...")
grad_val = grad_fn(m)
print(f"jax.grad: {grad_val}")

print("Computing finite differences...")
nu_plus = nominal_frequency_wrapper_fixed_init_reg(22.0)
nu_minus = nominal_frequency_wrapper_fixed_init_reg(20.0)
