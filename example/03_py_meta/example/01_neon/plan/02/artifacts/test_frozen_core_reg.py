...
# We define the energy functions under the Frozen Core Approximation with Norm Regularization
def initial_state_energy_frozen_reg(params, nuclear_mass):
...
def final_state_energy_frozen_reg(params, nuclear_mass):
    log_alpha_s = jnp.linspace(jnp.log(0.01), jnp.log(500.0), 8)
    log_alpha_p = jnp.linspace(jnp.log(0.05), jnp.log(100.0), 6)
    alpha_s = jnp.exp(log_alpha_s)
    alpha_p = jnp.exp(log_alpha_p)
    ...
    norm_penalty = 1.0 * (
        (jnp.dot(x_2p, jnp.dot(S_p_locked_avg, x_2p)) - 1.0)**2 +
        (jnp.dot(x_3p, jnp.dot(S_p_locked_avg, x_3p)) - 1.0)**2
    )
    return E_elec + norm_penalty

def nominal_frequency_wrapper_frozen_reg(nuclear_mass):
    ...
    solver_initial = jaxopt.LBFGS(fun=initial_state_energy_frozen_reg, maxiter=80, tol=1e-12, implicit_diff=True)
    res_initial = solver_initial.run(init_params_initial, nuclear_mass)
    E_initial = initial_state_energy_frozen_reg(res_initial.params, nuclear_mass)
    jax.debug.print("  Initial State: energy = {}, error = {}, iterations = {}", E_initial, res_initial.state.error, res_initial.state.iter_num)
    
    solver_final = jaxopt.LBFGS(fun=final_state_energy_frozen_reg, maxiter=80, tol=1e-12, implicit_diff=True)
    res_final = solver_final.run(init_params_final, nuclear_mass)
    E_final = final_state_energy_frozen_reg(res_final.params, nuclear_mass)
...
m = 21.0
grad_fn = jax.grad(nominal_frequency_wrapper_frozen_reg)
print("--- Computing jax.grad with Frozen Core + Norm Reg ---")
grad_val = grad_fn(m)
print(f"\njax.grad: {grad_val}")

print("\n--- Computing Finite Differences ---")
nu_plus = nominal_frequency_wrapper_frozen_reg(22.0)
nu_minus = nominal_frequency_wrapper_frozen_reg(20.0)
...
print("SUCCESS: test_isotope_shift_cross_check passed with Frozen Core + Norm Reg!")
