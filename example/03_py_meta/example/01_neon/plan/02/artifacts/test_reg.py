...
print("Running regularized LBFGS:")
solver = jaxopt.LBFGS(fun=initial_state_energy_reg, maxiter=200, tol=1e-10)
res = solver.run(params_initial, 20.18)
grad_val = jax.grad(initial_state_energy_reg)(res.params, 20.18)
grad_norm = jnp.sqrt(sum(jnp.sum(g**2) for g in grad_val.values()))
print(f"Regularized LBFGS: value = {res.state.value}, state.error = {res.state.error}, computed grad_norm = {grad_norm}")
