...
def nominal_frequency_wrapper_scipy(nuclear_mass):
    ...
    # Use ScipyMinimize L-BFGS-B
    solver_initial = jaxopt.ScipyMinimize(fun=initial_state_energy_reg, method="L-BFGS-B", maxiter=800, implicit_diff=True)
    ...
    solver_final = jaxopt.ScipyMinimize(fun=final_state_energy_reg, method="L-BFGS-B", maxiter=800, implicit_diff=True)
...
m = 21.0
grad_fn = jax.grad(nominal_frequency_wrapper_scipy)
print("Computing jax.grad with ScipyMinimize...")
grad_val = grad_fn(m)
print(f"jax.grad: {grad_val}")

print("Computing finite differences...")
nu_plus = nominal_frequency_wrapper_scipy(22.0)
nu_minus = nominal_frequency_wrapper_scipy(20.0)
