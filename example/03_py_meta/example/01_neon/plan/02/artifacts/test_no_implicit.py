...
# Let's define the nominal frequency wrapper with implicit_diff=False
from solver import initial_state_energy, final_state_energy, get_initial_guesses

def nominal_frequency_wrapper_no_implicit(nuclear_mass):
    x_1s_init, x_2s_init, x_5s_init, x_2p_init, x_3p_init = get_initial_guesses(nuclear_mass)
    ...
    # maxiter=150
    solver_initial = jaxopt.LBFGS(fun=initial_state_energy, maxiter=150, tol=1e-10, implicit_diff=False)
    ...
    solver_final = jaxopt.LBFGS(fun=final_state_energy, maxiter=150, tol=1e-10, implicit_diff=False)
...
m = 21.0
grad_fn = jax.grad(nominal_frequency_wrapper_no_implicit)
print("Computing jax.grad with implicit_diff=False...")
grad_val = grad_fn(m)
print(f"jax.grad: {grad_val}")

print("Computing finite differences...")
nu_plus = nominal_frequency_wrapper_no_implicit(22.0)
nu_minus = nominal_frequency_wrapper_no_implicit(20.0)
