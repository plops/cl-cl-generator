...
;; Let's define the generator template code
(defparameter *code*
  `(toplevel
     (in-package :cockpit)

     (comment "=====================================================")
     (comment " Generated bandwidth-optimized Linux TUI Cockpit      ")
     (comment "=====================================================")
     ...
     ;; Proc Parsers generated using generator loop
     ,@(loop for (name file-path fields parser-body) in
             `((parse-cpu-steal "/proc/stat" (steal idle total)
                 (let ((line (read-line stream nil)))
                   (when (and line (string= "cpu " (subseq line 0 4)))
                     (let* ((parts (uiop:split-string line :separator '(#\Space)))
                       ...
                       (values steal total)))))
               (parse-io-pressure "/proc/pressure/io" (some-avg10 full-avg10)
               ...
               (parse-vmstat "/proc/vmstat" (pswpin pswpout)
                 (let ((pswpin 0)
                       (pswpout 0))
             ...
             collect (build-proc-parser name file-path parser-body))

     (comment "Parse global network bytes from /proc/net/dev")
     (defun parse-net-dev ()
       (let ((rx 0) (tx 0))
         (with-open-file (stream "/proc/net/dev" :direction :input :if-does-not-exist nil)
           (when stream
     ...
     (comment "Parse socket inode mappings from /proc/net/tcp and udp")
     (defun parse-sockets ()
     ...
     (comment "Parse PIDs and link socket inodes to processes")
     (defun map-pids-to-sockets (socket-map)
       (let ((pid-map (make-hash-table :test 'equal)))
         (dolist (pid-dir (uiop:subdirectories "/proc/"))
     ...
     (comment "Parse process IO bytes from /proc/<pid>/io")
     (defun parse-pid-io (pid)
       (let ((io-file (format nil "/proc/~a/io" pid))
             (rchar 0)
             (wchar 0)
             (read-bytes 0)
             (write-bytes 0))
     ...
     (comment "Parse process OOM score")
     ...
     (comment "Track historical rate differentials for processes")
     (defparameter *last-proc-net* (make-hash-table :test 'equal))

     (defun calculate-proc-net-rates (pid socket-connections interval-sec)
       (let ((bytes 0))
         ;; Standard fallback if we cannot capture per-socket detail:
         ;; We approximate process network rate by analyzing read-bytes / write-bytes deltas
         ;; excluding actual file read-bytes/write-bytes if known.
         ;; For our cockpit, we'll map the sockets and count active sockets.
         ;; Let's return average estimates.
         (declare (ignore socket-connections interval-sec))
         (values bytes bytes)))

     (comment "Gather all process metrics")
     (defun gather-process-metrics (interval-sec)
       (let* ((socket-map (parse-sockets))
              (pid-sockets (map-pids-to-sockets socket-map))
              (processes nil))
         (maphash (lambda (pid sockets)
                    ...
                    (multiple-value-bind (rchar wchar read-b write-b) (parse-pid-io pid)
                      (let* ((pid-key (format nil "~a" pid))
                             (last-io (gethash pid-key *last-proc-net*))
                             (rx-rate 0)
                             (tx-rate 0))
                        (when last-io
                        ...
                        (setf (gethash pid-key *last-proc-net*) (list rchar wchar read-b write-b))
                        (push (make-process-info
                               :pid pid
                               :name (parse-pid-name pid)
                               :rx-rate rx-rate
                               :tx-rate tx-rate
                               :read-bytes read-b
                               :write-bytes write-b
     ...
     (comment "Delta-buffered UI Engine (saves SSH bandwidth)")
     (defparameter *current-buffer* (make-array '(24 80) :element-type 'character :initial-element #\Space))
     (defparameter *back-buffer* (make-array '(24 80) :element-type 'character :initial-element #\Space))

     (defun clear-screen-buffer ()
       (dotimes (y 24)
         (dotimes (x 80)
           (setf (aref *current-buffer* y x) #\Space))))
     ...
     (comment "Cgroup & Traffic Control throttling")
     ...
     (comment "Main Cockpit Loop")
     (defun run-cockpit ()
       (format t "~c[2J~c[H" #\Esc #\Esc) ;; Initial clear screen
       (force-output)
       (let ((cpu-history nil)
             (net-rx-history nil)
             (net-tx-history nil)
         ...
         (loop
           (clear-screen-buffer)
           (write-str-to-buffer 0 2 "=== Linux TUI Cockpit (Bandwidth-Optimiert) ===")

           (comment "CPU Steal Calculation & Plotting")
             ...
             (when (and steal total)
               (let* ((d-steal (- steal last-steal))
                 ...
                 (let ((spark (make-sparkline cpu-history :max-val 100)))
                   (write-str-to-buffer 2 2 (format nil "CPU Steal: ~5,2f% [~a]" steal-pct spark))))))
           
           (comment "IO Pressure (PSI)")
           (multiple-value-bind (some-io full-io) (parse-io-pressure)
             (write-str-to-buffer 4 2 (format nil "I/O Pressure Stall (PSI): SOME: ~a%, FULL: ~a%" (or some-io 0.0) (or full-io 0.0))))

           (comment "Swap Page In/Out Activity")
           (multiple-value-bind (pswpin pswpout) (parse-vmstat)
             (write-str-to-buffer 6 2 (format nil "Swap Page In/Out: pswpin: ~a, pswpout: ~a" pswpin pswpout)))

           (comment "Global Network Bandwidth rates")
           (multiple-value-bind (rx tx) (parse-net-dev)
             (when (and rx tx)
               (let* ((d-rx (if (= last-net-rx 0) 0 (- rx last-net-rx)))
                      (d-tx (if (= last-net-tx 0) 0 (- tx last-net-tx)))
                 ...
                 (write-str-to-buffer 8 2
                                      (format nil "Net Rate: RX: ~5,2f kB/s [~a]  TX: ~5,2f kB/s [~a]"
                                              rx-rate-kb (make-sparkline net-rx-history)
                                              tx-rate-kb (make-sparkline net-tx-history))))))

           (comment "Process lists")
...
(let ((path (write-source "cockpit" *code* *output-dir*)))
  (format t "Successfully generated TUI Cockpit source at ~a~%" path))
