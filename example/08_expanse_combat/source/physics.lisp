...
(defun propagate-state-2d (ad bd x u)
  (let ((x-next (mat-vec-mult-2d ad x)) (bu (mat-vec-mult-2d bd u)))
...
(defun update-torpedoes (torpedoes target-pos ad bd dt)
  ...
  (loop for torp in torpedoes
        collect (let* ((tx (first torp)) (ty (second torp)) (tvx (third torp))
                       (tvy (fourth torp)) (fuel (fifth torp)))
                  (if (> fuel 0.0)
                      (let* ((dx (- (aref target-pos 0) tx))
                             (dy (- (aref target-pos 1) ty))
                             (dist (sqrt (+ (* dx dx) (* dy dy)))) (acc 25.0d0)
                             (ux
                              (if (> dist 0.1)
                                  (* acc (/ dx dist))
                                  0.0d0))
                             (uy
                              (if (> dist 0.1)
                                  ...
                                  0.0d0))
                             (next-state
                              (propagate-state-2d ad bd (vector tx ty tvx tvy)
                               (vector ux uy))))
...
(defun update-slugs (slugs ad bd dt)
  ...
  (loop for slug in slugs
        collect (let* ((sx (first slug)) (sy (second slug)) (svx (third slug))
                       (svy (fourth slug))
                       (next-state
                        (propagate-state-2d ad bd (vector sx sy svx svy)
                         #(0.0d0 0.0d0))))
                  (list (aref next-state 0) (aref next-state 1)
                        (aref next-state 2) (aref next-state 3)))))
...
(defun update-pdc-defense (ship-pos torpedoes pdc-range dt)
  ...
  (let ((closest-torp nil) (min-dist 100000.0d0) (remaining-torpedoes nil)
        (bullet-trail nil))
    (dolist (torp torpedoes)
      (let* ((tx (first torp)) (ty (second torp)) (dx (- (aref ship-pos 0) tx))
    ...
    (if closest-torp
        (progn
          (setf bullet-trail
                  (list (aref ship-pos 0) (aref ship-pos 1)
