...
(defun init-game-gcs (win)
  ...
  (create-gc *gc-docking-port* :foreground 16763904)
  (create-gc *gc-ship-player* :foreground 3394611)
  (create-gc *gc-ship-enemy* :foreground 16724787)
  (create-gc *gc-weapon-torp* :foreground 16750848)
  (create-gc *gc-weapon-rail* :foreground 3394815)
  (create-gc *gc-pdc-bullets* :foreground 16777215))

(defstruct subsystem (name "") (health 100.0d0))

(defstruct app-state
  (ship-state
   (make-array 4 :element-type 'double-float :initial-contents
               '(0.0d0 -120.0d0 0.0d0 0.0d0))
  ...
  (slugs nil)
  (pdc-bullet nil)
  (enemy-pdc-bullet nil)
  (player-torpedoes nil)
  (player-slugs nil)
  (player-solver nil)
  (enemy-solver nil)
  (predicted-path nil)
...
(defun compile-tactical-shapes (state)
  (let ((shapes nil) (ship (app-state-ship-state state))
        (enemy (app-state-enemy-state state)))
    (let ((sx (aref ship 0)) (sy (aref ship 1)) (ex (aref enemy 0))
          (ey (aref enemy 1)))
      ...
      (when (app-state-pdc-bullet state)
        (let ((bullet (app-state-pdc-bullet state)))
          (push
           (list :line (first bullet) (second bullet) (third bullet)
                 (fourth bullet) :color *gc-pdc-bullets*)
           shapes)))
      ...
      (dolist (slug (app-state-slugs state))
        (let ((x (first slug)) (y (second slug)) (vx (third slug))
              (vy (fourth slug)))
          (push (list :disk x y 2.0d0 :color *gc-weapon-rail*) shapes)
          (push
           (list :line x y (+ x (* 1.5d0 vx)) (+ y (* 1.5d0 vy)) :color
                 *gc-weapon-rail*)
           shapes)))
      (dolist (torp (app-state-torpedoes state))
        (push
         (list :disk (first torp) (second torp) 3.0d0 :color *gc-weapon-torp*)
         shapes))
      ...
      (dolist (torp (app-state-player-torpedoes state))
        (push
         (list :disk (first torp) (second torp) 3.0d0 :color *gc-ship-player*)
         shapes))
      (when (app-state-predicted-path state)
        (push
         (list :poly-line (app-state-predicted-path state) :color
               *gc-ship-player*)
         shapes))
      (push (list :disk sx sy 4.5d0 :color *gc-ship-player*) shapes)
      (push
       (list :line sx sy (+ sx (* 1.5d0 (aref ship 2)))
             (+ sy (* 1.5d0 (aref ship 3))) :color *gc-ship-player*)
       shapes)
      (when (> (app-state-enemy-reactor state) 0.0d0)
        (push (list :disk ex ey 6.0d0 :color *gc-ship-enemy*) shapes)
        (push
         (list :line ex ey (+ ex (* 1.5d0 (aref enemy 2)))
               (+ ey (* 1.5d0 (aref enemy 3))) :color *gc-ship-enemy*)
         shapes))
      shapes)))
...
(defun update-game-state (state msg)
  (cond
    ...
    ((eq (car msg) :toggle-player-defend)
     (setf (app-state-player-auto-defend-p state)
             (not (app-state-player-auto-defend-p state)))
     state)
    ((eq (car msg) :toggle-enemy-defend)
     (setf (app-state-enemy-auto-defend-p state)
             (not (app-state-enemy-auto-defend-p state)))
     state)
    ...
    ((eq (car msg) :select-target)
     (setf (app-state-locked-target state) (cadr msg)) state)
    ((eq (car msg) :fire-railgun)
     (when (> (app-state-fuel state) 2.0d0)
       ...
       (let* ((ship (app-state-ship-state state))
         ...
         (when (> dist 0.1d0)
           (let* ((vel-slug 120.0d0) (vx (* vel-slug (/ dx dist)))
                  (vy (* vel-slug (/ dy dist))))
             (push (list (aref ship 0) (aref ship 1) vx vy)
                   (app-state-player-slugs state))))))
     state)
    ((eq (car msg) :fire-torpedo)
     (when (> (app-state-fuel state) 5.0d0)
       ...
       (let* ((ship (app-state-ship-state state))
              ...
              (dist (sqrt (+ (* dx dx) (* dy dy)))))
         (when (> dist 0.1d0)
           (let* ((vel-torp 30.0d0) (vx (* vel-torp (/ dx dist)))
                  (vy (* vel-torp (/ dy dist))))
             (push (list (aref ship 0) (aref ship 1) vx vy 6.0d0)
                   (app-state-player-torpedoes state))))))
     state)
    ((eq (car msg) :key-press)
     (let ((keysym (cadr msg)))
       (unless (app-state-autopilot-p state)
         (let* ((ship (app-state-ship-state state)) (thrust 8.0d0)
                (ux
                 (cond
                   ((eq keysym :up) thrust)
                   ((eq keysym :down) (- thrust))
                   (t 0.0d0)))
                (uy
                 (cond
                   ((eq keysym :right) thrust)
                   ((eq keysym :left) (- thrust))
                   (t 0.0d0))))
             ...
             (let ((ad
                   ...
                   (bd
                    (make-array '(4 2) :element-type 'double-float
                                :initial-element 0.0d0)))
               (multiple-value-bind (a b) (compute-cw-matrices-2d 0.00113d0
                                           0.05d0)
                 (setf ad a
                       bd b))
    ...
    ((eq (car msg) :tick)
     (if (app-state-game-over-p state)
         state
         (let* ((dt 0.05d0) (n 0.00113d0) (ship (app-state-ship-state state))
                (enemy (app-state-enemy-state state)) (ad nil) (bd nil))
           ...
           (if (app-state-player-auto-defend-p state)
               (multiple-value-bind (rem-torps
                                     bullet-trail) (update-pdc-defense ship
                                                    (app-state-torpedoes state)
                                                    45.0d0 dt)
           ...
           (if (and (> (app-state-enemy-reactor state) 0.0d0)
                    (app-state-enemy-auto-defend-p state))
               (multiple-value-bind (rem-torps
                                     bullet-trail) (update-pdc-defense enemy
                                                    (app-state-player-torpedoes
                                                     state)
                                                    45.0d0 dt)
           ...
           (setf (app-state-slugs state)
                   (update-slugs (app-state-slugs state) ad bd dt)
                 (app-state-torpedoes state)
                   (update-torpedoes (app-state-torpedoes state) ship ad bd dt)
                 (app-state-player-slugs state)
                   (update-slugs (app-state-player-slugs state) ad bd dt)
                 (app-state-player-torpedoes state)
                   (update-torpedoes (app-state-player-torpedoes state) enemy
                    ad bd dt))
           (dolist (slug (app-state-slugs state))
             (when (check-collision ship
                    (vector (first slug) (second slug) 0.0d0 0.0d0) 4.0d0)
               (setf (app-state-game-over-p state) t)))
           (dolist (torp (app-state-torpedoes state))
             (when (check-collision ship
                    (vector (first torp) (second torp) 0.0d0 0.0d0) 4.0d0)
               (setf (app-state-game-over-p state) t)))
           (when (> (app-state-enemy-reactor state) 0.0d0)
             (let ((tgt (app-state-locked-target state)))
               (dolist (slug (app-state-player-slugs state))
                 (when (check-collision enemy
                        (vector (first slug) (second slug) 0.0d0 0.0d0) 5.0d0)
                   (case tgt
               ...
               (dolist (torp (app-state-player-torpedoes state))
                 (when (check-collision enemy
                        (vector (first torp) (second torp) 0.0d0 0.0d0) 5.0d0)
                   (case tgt
                     ...
                     (:reactor
                      (decf (app-state-enemy-reactor state) 50.0d0)))))))
           (setf (app-state-enemy-fuel state)
                   (max 0.0d0 (app-state-enemy-fuel state))
                 (app-state-enemy-weapons state)
                   (max 0.0d0 (app-state-enemy-weapons state))
                 (app-state-enemy-radar state)
                   (max 0.0d0 (app-state-enemy-radar state))
                 (app-state-enemy-reactor state)
                   (max 0.0d0 (app-state-enemy-reactor state)))
           (if (and (app-state-autopilot-p state)
                    (> (app-state-fuel state) 0.0d0))
               (let* ((u-limit
                       (if (app-state-emergency-p state)
                           150.0d0
                           30.0d0))
                      (obs nil))
                 (dolist (slug (app-state-slugs state))
                 ...
                 (multiple-value-bind (u-traj x-pred status
                                       iterations) (solve-mpc-control
                                                    (app-state-player-solver
                                                     state)
                                                    ship #(0.0d0 0.0d0) obs
                                                    u-limit ship)
                   (declare (ignore status iterations))
                   (when u-traj
                     (let* ((u0 (aref u-traj 0)) (ux (aref u0 0))
                            (uy (aref u0 1))
                       ...
                       (setf (app-state-ship-state state)
                               (propagate-state-2d ad bd ship (vector ux uy)))
                       (when (> g-force 3.0d0)
                         (incf (app-state-crew-strain state)
                               (* 1.5d0 (- g-force 3.0d0))))
                       (setf (app-state-predicted-path state)
                               (loop for k below (length x-pred)
                                     collect (list (aref (aref x-pred k) 0)
                                                   (aref (aref x-pred k)
                                                         1))))))))
                    ...
                    (eu
                     (vector
                      ...
                      (if (> dist 0.1d0)
                          (* 10.0d0 (/ dy dist))
                          0.0d0))))
               (setf (app-state-enemy-state state)
                       (propagate-state-2d ad bd enemy eu))))
           (when (and (> (app-state-enemy-reactor state) 0.0d0)
                      (> (app-state-enemy-weapons state) 0.0d0)
                      (= 0
                         (mod (round (* (app-state-time-seconds state) 100))
                              200)))
             (let* ((blind-p (= (app-state-enemy-radar state) 0.0d0))
               ...
               (when (and (< dist 120.0d0) (> dist 0.1d0))
                 (if (< (random 1.0d0) 0.5d0)
                     (let* ((vel-slug
                            ...
                            (vx (* vel-slug (/ dx dist)))
                            (vy (* vel-slug (/ dy dist))))
                       (push (list (aref enemy 0) (aref enemy 1) vx vy)
                             (app-state-slugs state)))
                     (let* ((vel-torp 20.0d0) (vx (* vel-torp (/ dx dist)))
                            (vy (* vel-torp (/ dy dist))))
                       (push
                        (list (aref enemy 0) (aref enemy 1) vx vy
                              (if blind-p
                                  0.0d0
                                  8.0d0))
                        (app-state-torpedoes state)))))))
           ...
           (let* ((dist-to-port
                   (sqrt
                    (+ (* (aref ship 0) (aref ship 0))
                       (* (aref ship 1) (aref ship 1)))))
                  (speed
                   (sqrt
                    (+ (* (aref ship 2) (aref ship 2))
                       (* (aref ship 3) (aref ship 3))))))
           ...
           (setf (app-state-crew-strain state)
                   (max 0.0d0 (app-state-crew-strain state)))
           state)))
    (t state)))
  ...
  (let* ((n 15)
         ...
         (q-mat
          (make-array '(4 4) :element-type 'double-float :initial-element
                      0.0d0))
         (r-mat
          (make-array '(2 2) :element-type 'double-float :initial-element
                      0.0d0)))
    ...
    (let ((player-sol (init-mpc-solver n ad bd q-mat r-mat 30.0d0)))
      (run-gui #'update-game-state #'view
       (make-app-state :player-solver player-sol) :tick-interval 0.05d0
