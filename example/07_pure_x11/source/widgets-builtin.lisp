...
(register-widget "CANVAS"
 (lambda (w-struct focused pressed hovered)
   ...
   (let ((x (widget-x w-struct)) (y (widget-y w-struct))
     ...
     (draw-bevel x y w h :style :sunken)
     (let ((xmin (getf props :xmin -1.8)) (xmax (getf props :xmax 1.8))
           (ymin (getf props :ymin -1.8)) (ymax (getf props :ymax 1.8))
       ...
       (labels ((to-screen-x (wx)
                ...
                (draw-world-line (x1 y1 x2 y2 &key (gc *gc-text*))
                 (draw-line (to-screen-x x1) (to-screen-y y1) (to-screen-x x2)
               ...
               (case type
                 ...
                 (:text
                  (destructuring-bind (str tx ty &key (color *gc-text*)) args
                    (imagetext8 str :x (to-screen-x tx) :y (to-screen-y ty) :gc
                     color)))
                 (:poly-line
