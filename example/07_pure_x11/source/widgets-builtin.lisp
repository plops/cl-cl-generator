   ...
   (let ((x (widget-x w-struct)) (y (widget-y w-struct))
     ...
     (let* ((dw (- w 4)) (dh (- h 4)) (win-id *window*)
       ...
       (let ((*window* pix) (xmin (getf props :xmin -1.8))
             (xmax (getf props :xmax 1.8)) (ymin (getf props :ymin -1.8))
             (ymax (getf props :ymax 1.8)) (shapes (getf props :shapes nil))
         ...
         (labels ((to-screen-x (wx)
                  ...
                  (draw-world-line (x1 y1 x2 y2 &key (gc *gc-text*))
                   (draw-line (to-screen-x x1) (to-screen-y y1)
                    (to-screen-x x2) (to-screen-y y2) :gc gc))
                  (draw-world-circle (cx cy r &key (gc *gc-text*) fill-p)
                   (let* ((sx1 (to-screen-x (- cx r)))
                          (sy1 (to-screen-y (+ cy r)))
                          (sx2 (to-screen-x (+ cx r)))
                          (sy2 (to-screen-y (- cy r))) (sw (- sx2 sx1))
                          (sh (- sy2 sy1)))
                     (if fill-p
                         (poly-fill-arc (list (list sx1 sy1 sw sh 0 23040)) :gc
                          gc)
                         (poly-arc (list (list sx1 sy1 sw sh 0 23040)) :gc
                          gc)))))
           (declare
            (ignorable (function draw-world-line) (function draw-world-circle)))
           (let ((x-axis-y
                  (if (<= ymin 0 ymax)
                      0.0
                      ymin))
                 ...
                 (case type
                   ...
                   (:text
                    (destructuring-bind (str tx ty &key (color *gc-text*)) args
                      (imagetext8 str :x (to-screen-x tx) :y (to-screen-y ty)
                       :gc color)))
                   (:poly-line
                    (destructuring-bind (points &key (color *gc-text*)) args
                      (loop for (p1 p2) on points by #'cdr
                            while p2
                            do (draw-world-line (car p1) (cadr p1) (car p2)
                                (cadr p2) :gc color))))))))))
