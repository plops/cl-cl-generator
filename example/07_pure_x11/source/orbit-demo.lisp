...
(defun init-orbit-gcs (win)
  ...
  (create-gc *gc-sun* :foreground 16763904)
  (create-gc *gc-earth* :foreground 3381759)
  (create-gc *gc-mars* :foreground 16724736)
  (create-gc *gc-spacecraft* :foreground 3394611))

(defstruct app-state (time 0.0) (animating-p t))

(defparameter *orbit-trace-cache*
  (let ((a 1.262) (ecc 0.208))
    (loop for theta from 0.0 to (+ pi 0.05) by 0.05
          collect (let ((r
                         (/ (* a (- 1 (* ecc ecc))) (+ 1 (* ecc (cos theta))))))
  ...
  (case (car msg)
    (:tick
     (if (app-state-animating-p state)
         ...
         state))
    (:toggle-animation
     (make-app-state :time (app-state-time state) :animating-p
      (not (app-state-animating-p state))))
    (:reset-animation (make-app-state :time 0.0 :animating-p t))
    (t state)))

(defun get-planetary-shapes (t-val)
  (let* ((omega-e 1.0) (omega-m (/ 1.0 1.88))
         (mars-launch-phase (- pi (* omega-m pi))) (ex (cos (* omega-e t-val)))
         (ey (sin (* omega-e t-val)))
         (mx (* 1.524 (cos (+ mars-launch-phase (* omega-m t-val)))))
         (my (* 1.524 (sin (+ mars-launch-phase (* omega-m t-val))))) (a 1.262)
         (ecc 0.208)
         (r-space (/ (* a (- 1 (* ecc ecc))) (+ 1 (* ecc (cos t-val)))))
         (sx (* r-space (cos t-val))) (sy (* r-space (sin t-val)))
         (trace-points
          (loop for (th pt) in *orbit-trace-cache*
                while (<= th t-val)
                collect pt)))
    (list (list :disk 0.0 0.0 0.12 :color *gc-sun*)
...
(defun run-orbit-demo ()
  ...
  (run-gui #'update #'view (make-app-state) :tick-interval 0.05 :init-fn
   #'init-orbit-gcs))