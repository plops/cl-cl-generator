...
(defparameter *x11-requests*
  `(
    ...
    (:name make-window
     ...
     :bindings ((window (next-resource-id))
                (gc-light (next-resource-id))
                (gc-face (next-resource-id))
                (gc-shadow (next-resource-id))
                (gc-dark (next-resource-id))
                (gc-text (next-resource-id))
                (vals '(background-pixel border-pixel bit-gravity backing-store event-mask colormap))
                (n (length vals)))
    ...
    ;; 20. InternAtom
    (:name intern-atom
     :doc "Get or create an atom ID by string name."
     :params (name &key (only-if-exists 0))
     :decls ((declare (type string name)))
     :bindings ((n (length name))
                (p (pad n)))
     :packet ((card8 16)
              (card8 only-if-exists)
              (card16 (+ 2 (/ (+ n p) 4)))
              (card16 n)
              (card16 0)
              (string8 name)
    ...
    (:name change-property
     :doc "Change a property on a window (e.g. WM_PROTOCOLS)."
     :params (window property type data &key (mode 0) (format 32))
     :bindings ((data-list (if (listp data) data (list data)))
                (n (length data-list)))
     :packet ((card8 18)
              (card8 mode)
              (card16 (+ 6 n))
  ...
  '((expose
    ...
    (client-message
     ...
     :fields ((code (card8))
              ...
              (data32-3 (card32))
              (data32-4 (card32)))
     :returns (values format window type data32-0 data32-1 data32-2 data32-3 data32-4))
   ))
