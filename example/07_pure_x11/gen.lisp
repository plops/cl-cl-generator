...
;; 4. Generate widgets.lisp
(write-source "widgets"
  `(toplevel
     ,@(make-header-comments)
     (in-package :pure-x11-gen)
         ...
         (labels ((traverse (node)
                    (when (listp node)
                      (let ((w-struct (parse-node node)))
                        (when (member (widget-type w-struct) '(:button :checkbox :text-input))
     ...
     (defun run-gui (update-fn view-fn initial-state)
       ...
       (let ((win (make-window :width *window-width* :height *window-height*)))
         ...
         (let ((state initial-state)
               (layout nil)
               (keyboard-map (get-keyboard-mapping)))
                        ...
                        (when (= btn 1)
                          (let* ((hit (find-widget-at layout mx my))
                            ...
                            (when hit-name
                              ...
                              (if (member hit-type '(:button :checkbox :text-input))
                          ...
                          (cond
                            ...
                            (*focused-widget*
                             (let ((w-struct (find-widget-by-name layout *focused-widget*)))
                               (when (and w-struct (eq (widget-type w-struct) :text-input))
