...
(defparameter *widgets-core-template-code*
  `(toplevel
     ...
     (defun parse-node (node)
       ...
       (let ((type (let ((raw-type (car node)))
                     (if (symbolp raw-type)
                         (intern (symbol-name raw-type) :keyword)
                         raw-type)))
             (name nil)
     ...
     (defun resolve-layout (node &optional (default-x 0) (default-y 0) (default-w 0) (default-h 0))
       ...
       (when (listp node)
         (let ((w-struct (parse-node node)))
           (unless (widget-x w-struct) (setf (widget-x w-struct) default-x))
           (unless (widget-y w-struct) (setf (widget-y w-struct) default-y))
           (unless (widget-w w-struct) (setf (widget-w w-struct) default-w))
           (unless (widget-h w-struct) (setf (widget-h w-struct) default-h))
             ...
             (cond
               ((eq type :hbox)
                (let ((layout-results (compute-box-layout w-struct :x)))
                  (setf (widget-children w-struct)
                        (mapcar (lambda (item)
                                  (resolve-layout (first item) (second item) (third item) (fourth item) (fifth item)))
                                layout-results))))
               ((eq type :vbox)
               ...
               (t
                (setf (widget-children w-struct)
                      (mapcar (lambda (child)
                                (if (listp child)
                                    (resolve-layout child (widget-x w-struct) (widget-y w-struct) (widget-w w-struct) (widget-h w-struct))
                                    child))
         ...
         (labels ((traverse (w)
                    (when (widget-p w)
                      (let ((type (widget-type w)))
                        (when (and type (member type '(:button :checkbox :text-input) :test #'eq))
                          (push w result))
           ...
           (dolist (w widgets)
             (unless (eq (widget-name w) current-name)
               (let* ((tx (+ (widget-x w) (floor (widget-w w) 2)))
                      (ty (+ (widget-y w) (floor (widget-h w) 2)))
                      (dx (- tx cx))
                      (dy (- ty cy))
                      (dist (+ (* dx dx) (* dy dy))))
         ...
         (when (and (>= idx 0) (< idx (floor (length map-data) keysy-per-keycode)))
           (let* ((offset (* idx keysy-per-keycode))
             ...
             (cond
               ...
               ((= keysym-code #xff09) :tab)
               ((= keysym-code #xff0d) :return)
               ((= keysym-code #xff1b) :escape)
               ((= keysym-code #xff50) :home)
               ((= keysym-code #xff51) :left)
               ((= keysym-code #xff52) :up)
               ((= keysym-code #xff53) :right)
               ((= keysym-code #xff54) :down)
               ((= keysym-code #xff55) :page-up)
               ((= keysym-code #xff56) :page-down)
               ((= keysym-code #xff57) :end)
               ((= keysym-code #xffff) :delete)
               ((and (>= keysym-code #xffbe) (<= keysym-code #xffc9))
                (elt '(:f1 :f2 :f3 :f4 :f5 :f6 :f7 :f8 :f9 :f10 :f11 :f12)
                     (- keysym-code #xffbe)))
               (t nil))))))
     ...
     (defun render-widget (w-struct focused pressed hovered)
       ...
       (let* ((type (widget-type w-struct))
              (type-name (cond
                           ((stringp type) (string-upcase type))
                           ((symbolp type) (string-upcase (symbol-name type)))
                           (t nil)))
     ...
     (defun compute-box-layout (w-struct axis)
       ...
       (let* ((props (widget-props w-struct))
         ...
         (loop for child in children
               for size in sizes
               do (let* ((cx (if (eq axis :x) pos (+ container-x padding)))
